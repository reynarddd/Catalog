<template>
    <div class="bg-[#050E3C] block max-w-sm p-6 border border-default rounded-base shadow-xs">
        <a href="#">
            <img class="rounded-base" :src="products[0]?.thumbnail" alt="" />
        </a>
        <a href="#">
            <h5 class="mt-6 mb-2 text-2xl font-semibold tracking-tight text-heading">{{products[0]?.title || "Loading"}}</h5>
        </a>
        <p class="mb-6 text-body">{{products[0]?.description || "Loading"}}</p>
